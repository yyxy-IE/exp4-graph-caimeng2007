...
void DFSTraversal(Graph *g, int start) {
    ...
    //调用 DFSRecursive 从 start 开始遍历并输出序列
...
void connectivityAnalysis(Graph *g) {
    ...
    //遍历所有节点，如果未访问过，调用 DFSRecursive 进行深度优先遍历，并统计连通分量个数
    printf("连通分量分析结果：\n");
    for (int i = 0; i < g->vertexNum; i++) {
        if (!visited[i]) {
            componentCount++;
...
void dijkstra(Graph *g, int start, int *dist, int *prev) {
        ...
        for(int count = 0; count < g->vertexNum; count++) {
            ...
            while (e) {
                if (!visited[e->adjVex] && dist[u] != INT_MAX && dist[u] + e->weight < dist[e->adjVex]) {
...
void shortestPathByTransfer(Graph *g, int start, int end) {
    int *originalWeights = NULL;
    int weightCount = 0;
    for(int i = 0; i < g->vertexNum; i++) {
        EdgeNode *e = g->vertices[i].firstEdge;
        while (e) {
            if (g->vertices[i].lineCount > 1 || g->vertices[e->adjVex].lineCount > 1) {
                if (!originalWeights) {
                    originalWeights = (int*)malloc(sizeof(int) * g->edgeNum);
                }
                originalWeights[weightCount++] = e->weight;
                e->weight = 1;
            }
            e = e->next;
        }
